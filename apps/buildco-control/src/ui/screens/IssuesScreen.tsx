            ...
            <>
              <Toolbar label="Issue and delay filters">
                ...
                <Button
                  ...
                  onClick={() => editor.set({ kind: delay ? "delay" : "issue", action: "create", projectId: this.field("project").get() })}
                />
              </Toolbar>
                ...
                <div className="filter-grid">
                  ...
                  {!delay && (
                    <>
                      ...
                      {this.select("costView", "Cost view", [{ value: "overEstimate", label: "Actual exceeds estimate" }])}
                    </>
                  )}
                ...
                <DetailView
                  ...
                  actions={
                    <Toolbar label="Record actions">
                      <Button
                        label="Edit record"
                        onClick={() => editor.set({ kind: delay ? "delay" : "issue", action: "edit", row: result.detail!.record })}
                      />
                      <Button
                        label={delay ? "End delay" : "Resolve issue"}
                        disabled={["ended", "resolved", "closed"].includes(String(result.detail.record?.status))}
                        onClick={() => editor.set({ kind: delay ? "delay" : "issue", action: "resolve", row: result.detail!.record })}
                      />
                    </Toolbar>
                  }
                />
