...
class DemoControls extends Component {
  ...
  render(): FrayChild {
    ...
    return <aside className="demo-controls fray-size-natural" aria-label="Demo controls"><Panel island><details><summary><span><strong>Demo controls</strong><span className="muted">Theme, data, and component states</span></span><span className="demo-context">{transport === "embedded" ? "Embedded worker" : "HTTP service"} · Seed {b?.metadata.seed ?? "18431"} · {b?.projects.length ?? "…"} projects</span></summary>
...
class BuildCoApp extends FrayApp {
  ...
  protected renderContent(): FrayChild {
    ...
    return <div className="buildco fray-layout-vertical fray-size-flexible"><a className="skip-link" href="#main-content">Skip to content</a><header className="app-header fray-size-natural"><div className="brand-row"><a className="brand" href={router.href(routeTarget(routes.overview))}><span className="brand-mark" aria-hidden="true">B<span>c</span></span><span>BuildCo <b>Control</b><small>CONSTRUCTION OPERATIONS</small></span></a><div className="header-meta"><span className="live-dot" />Operational workspace<span className="profile-avatar" title="Demo workspace">BC</span></div></div>
      <NavigationBar label="Main navigation" allocation="natural" items={SCREENS.map((screen, i) => ({ id: screen, label: labels[i]!, to: routeTarget(routes[screen]) }))} />
    </header><main id="main-content" className="fray-layout-vertical fray-size-flexible" tabIndex={-1}>
      {!b.value && <Panel key="startup" island allocation="natural"><div className="startup" role="status"><span className="eyebrow">BuildCo Control</span><h1>{b.fetchState === "error" ? "The scenario could not be loaded" : "Preparing your operational workspace"}</h1><p>{b.fetchState === "error" ? String(b.error) : "Generating projects, crews, materials, and their connected histories. The full demo may take about 30 seconds."}</p>{b.fetchState === "error" ? <Button label="Retry startup" onClick={() => void bootstrap.retry()} /> : <ProgressBar label="Preparing scenario" valueEmitter={demo.progress} />}</div></Panel>}
      <div key="workspace" className="fray-layout-vertical fray-size-flexible" hidden={!b.value}><fieldset className="workspace-fieldset fray-layout-vertical fray-size-flexible" disabled={disabled} inert={disabled}><legend className="sr-only">Construction workspace</legend><RouteOutlet initialActiveViewId="overview" mountPolicy="active-only" views={this.views} /></fieldset></div>
      <DemoControls key="demo-controls" />
    </main><footer className="app-footer fray-size-natural"><span>BuildCo Control <span className="muted">/ Connected construction operations</span></span><span>Deterministic demo · All costs in DKK · Working-day schedules</span></footer></div>;
  }
}
...
mountFrayApp(createFrayRuntime({ router }), BuildCoApp, document.getElementById("app")!, { sizing: "viewport", layout: "vertical", landmark: "none" });
