...
export class RecordEditor extends Component {
    ...
    initialize(): void {
        ...
        this.onCleanup(this.fields.project!.subscribe(() => {
            if (editor.get()?.action === "create") this.fields.phase!.set("");
            void this.loadChoices();
        }, {emitCurrent: false}));
    }
        ...
        return <Dialog title={title} valueEmitter={this.open}
            ...
            {resolving ? <p>Mark “{current?.row?.name}”
                ...
                <div className="editor-fields">
                    <Dropdown key={`projects-${bootstrap.get()?.metadata.seed}`} label="Project"
                              valueEmitter={this.fields.project!} options={bootstrap.get()?.choices.projects ?? []}
                              disabled={current?.action === "edit" || busy} required/>
                    <Dropdown label="Phase" placeholder="Choose a phase…" valueEmitter={this.fields.phase!}
                              options={this.choices} disabled={current?.action === "edit" || busy} required/>
                    <Textbox label={delay ? "Delay description" : "Issue title"} valueEmitter={this.fields.title!}
                             required maxLength={180} disabled={busy}/>
                    {!delay && <><Textbox label="Description" valueEmitter={this.fields.description!}
                                          disabled={busy}/><Dropdown label="Severity"
                                                                     valueEmitter={this.fields.severity!}
                                                                     options={["low", "medium", "high", "critical"].map(value => ({
                                                                         value,
                                                                         label: human(value)
                                                                     }))} disabled={busy}/><Dropdown
                        label="Assigned person" valueEmitter={this.fields.person!}
                        options={[{value: "__none", label: "Unassigned"}, ...bootstrap.get()?.choices.people ?? []]}
                        disabled={busy}/><Textbox label="Due date" type="date" valueEmitter={this.fields.due!}
                                                  disabled={busy}/><Textbox label="Estimated cost · DKK" type="number"
                                                                            valueEmitter={this.fields.estimate!}
                                                                            disabled={busy}/></>}
                    {delay && <><Textbox label="Estimated lost hours" type="number"
                                         valueEmitter={this.fields.lostHours!}
                                         disabled={busy || current?.action === "edit"}/><Textbox
                        label="Estimated schedule impact · days" type="number" valueEmitter={this.fields.impactDays!}
                        disabled={busy || current?.action === "edit"}/></>}
                    {current?.action === "create" &&
                        <p className="muted">{delay ? "Recorded as an active, reduced-capacity delay with an unclassified cause." : "Recorded as an open issue with an unknown, suspected cause. No actual cost is invented; actual costs come from attributed resource entries."}</p>}
                </div>}
        </Dialog>;
    }
    ...
    private async loadChoices(): Promise<void> {
        ...
        try {
            const result = await buildco.choices(this.fields.project!.get());
            if (id === this.requestId) this.choices.set(result.phases ?? []);
        } catch (error) {
            if (id === this.requestId) this.error.set(String(error));
        }
    }
    ...
    private async save(): Promise<void> {
        ...
        if (!current) return;
        if (current.action !== "resolve" && (!this.fields.phase!.get() || this.fields.title!.get().trim().length < 3)) {
            this.error.set("Choose a phase and enter a title with at least 3 characters.");
            return;
        }
        const result = await mutate.run({
            ...
            id: current.row?.id,
            projectId: this.fields.project!.get(),
            phaseId: this.fields.phase!.get(),
            title: this.fields.title!.get(),
            description: this.fields.description!.get(),
            severity: this.fields.severity!.get(),
            personId: this.fields.person!.get() === "__none" ? undefined : this.fields.person!.get() || undefined,
            dueDate: this.fields.due!.get() || undefined,
            estimatedCost: Number(this.fields.estimate!.get()),
            lostHours: Number(this.fields.lostHours!.get()),
            impactDays: Number(this.fields.impactDays!.get())
        });
