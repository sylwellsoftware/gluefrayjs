import type { CivilDate, MaterialCategory, MaterialUnit, PersonnelCategory, PhaseCategory, PhasePlan, PhasePlanId, Project, ProjectType, ScopeNode, ScenarioData, ScenarioGenerationOptions } from "../domain/model.ts";
import { addDays, addMonths, at, civilDate, daysBetween, maxDate, workingDaysInclusive, workingOnOrAfter } from "../domain/calendar.ts";
import { byId, groupBy, round, sum } from "../domain/collections.ts";
import { scheduleBaseline } from "../domain/dependencies.ts";
import { IdSequence, type GenerationContext, type ProjectFactors } from "./state.ts";
...
interface Recipe {
  name: string; category: PhaseCategory; trade: number; hours: number;
...
export function createPlan(options: ScenarioGenerationOptions): GenerationContext {
  ...
  const metadata = { schemaVersion: 1 as const, generatorVersion: 1 as const, seed: options.seed,
  ...
  const data: ScenarioData = { metadata, materialTypes: [], personnelTypes: [], people: [], personnelCostRates: [],
    suppliers: [], customers: [], phaseTypes: [], projects: [], workingCalendars: [], scopeNodes: [], projectPlans: [],
    phasePlans: [], phaseDependencies: [], labourBudgetLines: [], materialBudgetLines: [], otherBudgetLines: [], milestonePlans: [],
    personnelAssignments: [], personnelAbsences: [], siteReports: [], progressReports: [], labourEntries: [], materialUsageEntries: [],
    purchaseOrders: [], purchaseOrderLines: [], deliveries: [], deliveryLines: [], issues: [], issueCauses: [], otherCostEntries: [],
    delayEvents: [], delayImpactReports: [], milestoneReports: [] };
  const ctx: GenerationContext = { data, rng: new Random(options.seed), ids: new IdSequence(), factors: new Map(), phases: new Map(), purchases: [] };
...
function createReferenceData({ data, rng, ids }: GenerationContext): void {
  ...
  data.workingCalendars.push({ id: ids.next("calendar"), name: "Demo weekday calendar", workingDays: [1, 2, 3, 4, 5], regularHoursPerDay: 7.4, holidays });
  for (const [name, category, unit, cost] of MATERIALS) data.materialTypes.push({
    id: ids.next("materialType"), code: `MAT-${data.materialTypes.length + 1}`, name, category, unit,
    standardUnitCost: cost, defaultLeadTimeDays: rng.int(5, 18), defaultWasteAllowancePct: category === "HVAC" ? 1 : rng.int(2, 5),
    criticality: category === "electrical" || category === "HVAC" ? "critical" : "normal",
  });
  for (const [name, category, rate] of TRADES) data.personnelTypes.push({ id: ids.next("personnelType"), name, category,
    defaultNominalHoursPerWeek: 37, planningRegularHourlyCost: rate, planningOvertimeHourlyCost: round(rate * 1.5, 2), maxRecommendedHoursPerWeek: 48 });
  const headcount = Math.max(27, Math.round(({ small: 54, demo: 432, stress: 1620 }[data.metadata.profile]) * data.metadata.projectCount / ({ small: 5, demo: 40, stress: 180 }[data.metadata.profile])));
  for (let i = 0; i < headcount; i++) {
    const trade = data.personnelTypes[i % TRADES.length]!;
    const subcontractor = rng.chance(0.15);
    const person = { id: ids.next("person"), name: `${rng.pick(FIRST_NAMES)} ${rng.pick(LAST_NAMES)} ${i + 1}`,
    ...
    data.personnelCostRates.push({ id: ids.next("costRate"), personId: person.id, validFrom: addMonths(data.metadata.simulationStart, -12),
      validTo: addDays(change, -1), regularHourlyCost: regular, overtimeHourlyCost: round(regular * 1.5, 2) });
    data.personnelCostRates.push({ id: ids.next("costRate"), personId: person.id, validFrom: change,
  ...
  for (let i = 0; i < 24; i++) data.suppliers.push({ id: ids.next("supplier"), name: `${["Nordic", "Harbour", "Baltic", "Central"][i % 4]} ${categories[i % categories.length]} Supply ${i + 1}`,
    materialCategories: [categories[i % categories.length]!], typicalLeadTimeDays: rng.int(5, 15),
    reliabilityRating: i < 10 ? 0.98 : rng.normal(0.78, 0.06), qualityRating: i < 10 ? 0.99 : 0.82, region: "Denmark" });
  for (let i = 0; i < 18; i++) data.customers.push({ id: ids.next("customer"), name: `${PLACES[i % PLACES.length]} Development ${i + 1}`,
    type: i % 3 === 0 ? "publicAuthority" : i % 3 === 1 ? "industrial" : "residentialDeveloper" });
  for (const recipe of RECIPES) data.phaseTypes.push({ id: ids.next("phaseType"), name: recipe.name, category: recipe.category, typicalOutputUnit: "squareMetre" });
}
...
function createProject(ctx: GenerationContext, index: number): void {
  ...
  const factors: ProjectFactors = { cohort, story, productivity: story === "healthy" ? 1.14 : story === "quality" ? 0.77 : 0.95,
  ...
  const supervisors = data.people.filter(p => p.personnelTypeId === data.personnelTypes[8]!.id);
  const project: Project = { id: ids.next("project"), code: `BC-${String(index + 1).padStart(3, "0")}`,
    name: `${PLACES[index % PLACES.length]} ${Math.floor(index / PLACES.length) + 1}`, projectType: PROJECT_TYPES[index % PROJECT_TYPES.length]!,
    customerId: data.customers[index % data.customers.length]!.id, contractType: rng.pick(["fixedPrice", "costPlus", "targetCost"]),
    status: "planned", priority: story === "supply" ? "high" : "normal", complexity: story === "quality" ? "high" : "medium",
    baselineRisk: story === "healthy" ? "low" : "medium", location: rng.pick(["Copenhagen", "Aarhus", "Odense", "Aalborg"]),
    region: "Denmark", currency: "DKK", projectManagerId: supervisors[index % supervisors.length]!.id,
    siteManagerId: supervisors[(index + 1) % supervisors.length]!.id, plannedStart: data.metadata.anchorDate, plannedCompletion: data.metadata.anchorDate,
    workingCalendarId: calendar.id, rules: { nightWorkAllowed: false, weekendWorkAllowed: story !== "healthy", occupiedSite: cohort === "active" && index % 3 === 0,
      restrictedAccess: story === "quality", securityClearanceRequired: false, noiseRestrictions: index % 2 === 0, environmentalRestrictions: index % 3 === 0 } };
  data.projects.push(project);
  ctx.factors.set(project.id, factors);
  const plan = { id: ids.next("projectPlan"), projectId: project.id, version: 1, status: "approved" as const, createdAt: at(data.metadata.simulationStart) };
  data.projectPlans.push(plan);
  const root: ScopeNode = { id: ids.next("scope"), projectId: project.id, type: "site", name: project.name, sequence: 0 };
  ...
  const createPhase = (scope: ScopeNode, recipeIndex: number, scale: number): PhasePlan => {
    ...
    const phase: PhasePlan = { id: ids.next("phase"), projectPlanId: plan.id, projectId: project.id, scopeNodeId: scope.id,
      phaseTypeId: data.phaseTypes[recipeIndex]!.id, name: recipe.name, plannedStart: project.plannedStart, plannedFinish: project.plannedStart,
      progressProfile: "linear", progressWeight: hours, plannedOutputQuantity: scope.areaM2 ?? scope.lengthM ?? 100,
      outputUnit: scope.lengthM ? "metre" : "squareMetre", priority: project.priority };
    phases.push(phase);
    durations.set(phase.id, Math.max(2, Math.ceil(hours / (headcount * calendar.regularHoursPerDay))));
    data.labourBudgetLines.push({ id: ids.next("labourBudget"), phasePlanId: phase.id, personnelTypeId: trade.id,
      estimatedHours: hours, plannedHeadcount: headcount, budgetRegularHourlyCost: trade.planningRegularHourlyCost, budgetOvertimeHourlyCost: trade.planningOvertimeHourlyCost });
    for (const [materialIndex, quantity] of recipe.materials) {
      ...
      data.materialBudgetLines.push({ id: ids.next("materialBudget"), phasePlanId: phase.id, materialTypeId: material.id,
        requiredQuantity, plannedWastePct: material.defaultWasteAllowancePct, budgetedQuantity: round(requiredQuantity * (1 + material.defaultWasteAllowancePct / 100)),
        budgetUnitCost: material.standardUnitCost, requiredByDate: project.plannedStart });
    }
    return phase;
  };
  const link = (pred: PhasePlan, successor: PhasePlan, type: "finishToStart" | "startToStart" | "finishToFinish" = "finishToStart", lag = 0): void => {
    dependencies.push({ id: ids.next("dependency"), predecessorPhaseId: pred.id, successorPhaseId: successor.id, type, lagWorkingDays: lag });
  };
  ...
  for (let building = 0; building < 2; building++) {
    const container: ScopeNode = { id: ids.next("scope"), projectId: project.id, parentId: root.id, type: "building",
      name: `${project.projectType === "highRise" ? "Tower" : project.projectType === "factory" ? "Production hall" : "Section"} ${building === 0 ? "A" : "B"}`, sequence: building };
    ...
    for (let n = building; n < leafCount; n += 2) {
      ...
      const scope: ScopeNode = { id: ids.next("scope"), projectId: project.id, parentId: container.id, type,
        name: `${type[0]!.toUpperCase()}${type.slice(1)} ${String(Math.floor(n / 2) + 1).padStart(2, "0")}`, sequence: n,
        areaM2: type === "segment" ? undefined : rng.int(200, 450), lengthM: type === "segment" ? rng.int(100, 300) : undefined,
      ...
      if (project.projectType === "infrastructure") {
        const excavation = createPhase(scope, 11, scale); link(foundation, excavation);
        const structure = createPhase(scope, 1, scale); link(excavation, structure);
        const utilities = createPhase(scope, 5, scale); link(structure, utilities);
        const testing = createPhase(scope, 14, scale); link(utilities, testing);
        continue;
      }
      const structure = createPhase(scope, 1, scale); link(foundation, structure);
      if (previousStructure && type === "floor") link(previousStructure, structure);
      previousStructure = structure;
      const walls = createPhase(scope, 2, scale); link(structure, walls);
      const windows = createPhase(scope, 3, scale); link(walls, windows);
      const electrical = createPhase(scope, 4, scale); link(walls, electrical);
      const plumbing = createPhase(scope, 5, scale); link(walls, plumbing, "startToStart", 1);
      const hvac = createPhase(scope, 6, scale); link(walls, hvac); link(electrical, hvac, "finishToFinish");
      const interior = createPhase(scope, 7, scale);
      for (const pred of [windows, electrical, plumbing, hvac]) link(pred, interior);
      const finishing = createPhase(scope, 10, scale); link(interior, finishing);
      const commissioning = createPhase(scope, 8, scale); link(finishing, commissioning);
    }
    if (project.projectType !== "infrastructure") {
      const roof: ScopeNode = { id: ids.next("scope"), projectId: project.id, parentId: container.id, type: "roof", name: "Roof", sequence: leafCount, areaM2: 500 };
      data.scopeNodes.push(roof);
      const roofing = createPhase(roof, 9, 1.5); link(previousStructure ?? foundation, roofing);
    }
  }
  ...
  data.phasePlans.push(...phases); data.phaseDependencies.push(...dependencies);
  ...
  ] as const) if (subset.length) data.milestonePlans.push({ id: ids.next("milestone"), projectPlanId: plan.id, projectId: project.id,
    name, category, plannedDate: maxDate(...subset.map(p => p.plannedFinish)), critical: category === "handover", phasePlanIds: subset.map(p => p.id) });
  data.otherBudgetLines.push({ id: ids.next("otherBudget"), projectId: project.id, category: "temporaryFacilities", description: "Site setup and facilities", budgetedAmount: 12000 });
}
...
function initializePhasesAndAssignments(ctx: GenerationContext): void {
  ...
  for (const phase of [...data.phasePlans].sort((a, b) => a.plannedStart < b.plannedStart ? -1 : a.plannedStart > b.plannedStart ? 1 : a.id < b.id ? -1 : 1)) {
    ...
    const calendar = data.workingCalendars[0]!;
    const state = { plan: phase, project, calendar, factors: ctx.factors.get(project.id)!, labour: labour.get(phase.id)!, materials: materials.get(phase.id)!,
      assignments: [] as ScenarioData["personnelAssignments"], dependencies: dependencies.get(phase.id) ?? [],
      progress: 0, productiveHours: 0, rework: [], forecastStart: phase.plannedStart, forecastFinish: phase.plannedFinish,
      lastReportedProgress: 0, injectedIssue: false, injectedAbsence: false, injectedWeather: false, openDelays: new Map() };
    ...
    for (const budget of state.labour) {
      ...
      for (const person of candidates.slice(0, budget.plannedHeadcount)) {
        const assignment = { id: ids.next("assignment"), personId: person.id, projectId: project.id, phasePlanId: phase.id,
          startDate: phase.plannedStart, createdAt: at(data.metadata.simulationStart, 0), plannedHoursPerWeek: Math.min(person.nominalHoursPerWeek,
            sum(state.labour, b => b.estimatedHours) / workingDaysInclusive(phase.plannedStart, phase.plannedFinish, calendar) * 5), role: "Production crew" };
        state.assignments.push(assignment); data.personnelAssignments.push(assignment);
