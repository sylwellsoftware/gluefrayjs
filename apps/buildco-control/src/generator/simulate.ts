import type { CivilDate, DelayCauseType, DelayEvent, Id, Issue, IssueCauseCategory, LabourEntry, MaterialBudgetLine, PersonnelAbsence, PhaseExecutionStatus, ProjectId, ScenarioData, Severity } from "../domain/model.ts";
import { addDays, addWorkingDays, at, daysBetween, isWorkingDay, isoWeek, maxDate, workingOnOrAfter } from "../domain/calendar.ts";
import { byId, compare, groupBy, round, sum } from "../domain/collections.ts";
import { topologicalPhases } from "../domain/dependencies.ts";
import { inventoryKey, type GenerationContext, type Inventory, type PhaseState, type PlannedPurchase } from "./state.ts";
...
class Simulation {
  ...
  async run(): Promise<void> {
    ...
    if (trainingDate <= this.data.metadata.simulationEnd) this.data.personnelAbsences.push({ id: this.ctx.ids.next("absence"), personId: supervisor.id,
      fromDate: trainingDate, toDate: addDays(trainingDate, 2), type: "training", planned: true, notes: "Planned supervisor training; no production assignment" });
    ...
    for (this.date = this.data.metadata.simulationStart; this.date <= this.data.metadata.simulationEnd; this.date = addDays(this.date, 1)) {
      this.dayHours.clear(); this.siteReports.clear(); this.touchedDelays.clear();
      if (week !== isoWeek(this.date)) { week = isoWeek(this.date); this.weekHours.clear(); }
  ...
  private getStock(state: PhaseState, budget: MaterialBudgetLine): Inventory {
    ...
    if (!stock) { stock = { quantity: 0, value: 0 }; this.stock.set(key, stock); }
    return stock;
  }
  ...
  private purchase(state: PhaseState, budget: MaterialBudgetLine, quantity: number, orderedOn: CivilDate, requiredBy: CivilDate, initial: boolean): void {
    ...
    const order = { id: ids.next("purchaseOrder"), projectId: state.project.id, supplierId: supplier.id,
      orderedAt: at(orderedOn, 7), status: "ordered" as const, createdById: state.project.siteManagerId };
    const line = { id: ids.next("purchaseOrderLine"), purchaseOrderId: order.id, materialTypeId: budget.materialTypeId,
    ...
    const purchase: PlannedPurchase = { order, line, deliveryDate, rejectFraction: rejected ? 0.15 : 0,
      partialFraction: partial ? 0.65 : 1, emitted: false, receivedQuantity: 0, acceptedQuantity: 0 };
    this.ctx.purchases.push(purchase);
    const key = inventoryKey(state.project.id, budget.materialTypeId);
    const group = this.purchaseGroups.get(key) ?? []; group.push(purchase); this.purchaseGroups.set(key, group);
  ...
  private schedule(map: Map<CivilDate, PlannedPurchase[]>, date: CivilDate, purchase: PlannedPurchase): void {
    const entries = map.get(date) ?? []; entries.push(purchase); map.set(date, entries);
  }
  private emitOrder(purchase: PlannedPurchase): void {
    purchase.emitted = true; this.data.purchaseOrders.push(purchase.order); this.data.purchaseOrderLines.push(purchase.line);
  }
  private processOrdersAndDeliveries(): void {
    ...
    for (const p of this.deliverySchedule.get(this.date) ?? []) {
      ...
      const delivery = { id: this.ctx.ids.next("delivery"), purchaseOrderId: p.order.id, deliveredAt: at(this.date, 8),
        status: rejected > 0 ? "partRejected" as const : "accepted" as const, deliveryReference: `DEL-${p.order.id}-${this.date}` };
      this.data.deliveries.push(delivery);
      this.data.deliveryLines.push({ id: this.ctx.ids.next("deliveryLine"), deliveryId: delivery.id, purchaseOrderLineId: p.line.id,
      ...
      stock.quantity = round(stock.quantity + accepted); stock.value += accepted * p.line.unitCost;
      p.receivedQuantity = round(p.receivedQuantity + qty); p.acceptedQuantity = round(p.acceptedQuantity + accepted);
      p.order.status = p.receivedQuantity >= p.line.quantityOrdered - 0.0001 ? "delivered" : "partDelivered";
      if (p.order.status !== "delivered") {
        p.deliveryDate = addDays(this.date, 4); p.rejectFraction = 0;
  ...
  private processAbsencesAndWeather(): void {
    ...
    for (const person of this.data.people) {
      ...
      const absence: PersonnelAbsence = { id: this.ctx.ids.next("absence"), personId: person.id, fromDate: this.date,
        toDate: addDays(this.date, this.ctx.rng.int(1, 4)), type: "sickness", planned: false };
      this.absent.set(person.id, absence); this.data.personnelAbsences.push(absence);
    }
    for (const state of this.order) {
      ...
      if (!this.incidentProjects.has(state.project.id) && state.factors.story !== "healthy") {
        ...
        if (!this.absent.has(person)) {
          const absence: PersonnelAbsence = { id: this.ctx.ids.next("absence"), personId: person, fromDate: this.date,
            toDate: addDays(this.date, 4), type: "sickness", planned: false, notes: "Crew absence during active production" };
          this.absent.set(person, absence); this.data.personnelAbsences.push(absence);
        }
      ...
      if (!this.weatherProjects.has(state.project.id) && state.factors.story === "ordinary" && daysBetween(state.started, this.date) >= 8) {
        this.weatherUntil.set(state.project.id, addDays(this.date, 3)); this.weatherProjects.add(state.project.id);
      }
    }
  }
  ...
  private execute(state: PhaseState): void {
    ...
    if (!this.setupProjects.has(state.project.id)) {
      this.data.otherCostEntries.push({ id: this.ctx.ids.next("otherCost"), projectId: state.project.id, date: this.date,
        category: "temporaryFacilities", description: "Site setup and facilities", amount: state.factors.story === "healthy" ? 10500 : 12000, reason: "plannedWork" });
    ...
    for (const rework of state.rework) {
      ...
      if (rework.hoursLeft <= 0.0001) {
        rework.issue.status = "resolved"; rework.issue.resolvedAt = at(this.date, 17);
      } else {
    ...
    for (const budget of state.materials) {
      ...
      if (supported < delta) { delta = supported; limitingMaterial = budget; }
    }
    ...
    if (actualDelta > 0.00000001) {
      ...
      if (forced || this.ctx.rng.chance(state.factors.qualityRisk)) {
        ...
        this.createIssue(state, cause, severity); state.injectedIssue = true;
    ...
    if (state.progress >= 1 - 1e-8 && !state.rework.some(r => r.hoursLeft > 0.0001) && !finishBlocked) {
      state.progress = 1; state.finished = this.date;
      for (const assignment of state.assignments) if (!assignment.endDate) {
        assignment.endDate = this.date; assignment.endDateRecordedAt = at(this.date, 17);
      }
    }
  ...
  private allocate(state: PhaseState, requested: number, reason: LabourEntry["activityReason"], issueId?: Id<"issue">, delayEventId?: Id<"delay">): number {
    ...
    if (state.factors.story === "healthy") for (const _person of unavailable) {
      ...
      if (replacement) {
        ...
        const assignment = { id: this.ctx.ids.next("assignment"), personId: replacement.id, projectId: state.project.id, phasePlanId: state.plan.id,
          startDate: this.date, endDate: this.date, createdAt: at(this.date, 9), plannedHoursPerWeek: 37, role: "Temporary cover" };
        state.assignments.push(assignment); this.data.personnelAssignments.push(assignment);
      }
    }
    ...
    for (const person of new Map(candidates.map(p => [p.id, p])).values()) {
      ...
      this.dayHours.set(person.id, round(daily + hours)); this.weekHours.set(person.id, round(weekly + hours));
      this.data.labourEntries.push({ id: this.ctx.ids.next("labour"), siteReportId: this.siteReport(state), personId: person.id,
        projectId: state.project.id, phasePlanId: state.plan.id, workDate: this.date, hours, activityReason: reason, issueId, delayEventId });
      remaining -= hours;
    }
  ...
  private consume(state: PhaseState, budget: MaterialBudgetLine, quantity: number, reason: "plannedWork" | "waste" | "defectResolution", issueId?: Id<"issue">): void {
    ...
    this.data.materialUsageEntries.push({ id: this.ctx.ids.next("materialUsage"), siteReportId: this.siteReport(state),
    ...
    stock.value = Math.max(0, stock.value - quantity * cost); stock.quantity = round(Math.max(0, stock.quantity - quantity));
  }

  private siteReport(state: PhaseState): Id<"siteReport"> {
    let id = this.siteReports.get(state.project.id);
    if (!id) {
      id = this.ctx.ids.next("siteReport"); this.siteReports.set(state.project.id, id);
      this.data.siteReports.push({ id, projectId: state.project.id, reportDate: this.date, shift: "day", reporterId: state.project.siteManagerId, createdAt: at(this.date, 17) });
    }
    return id;
  }
  ...
  private createIssue(state: PhaseState, cause: IssueCauseCategory, severity: Severity, purchaseOrderLineId?: Id<"purchaseOrderLine">): void {
    ...
    const issue: Issue = { id: this.ctx.ids.next("issue"), projectId: state.project.id, scopeNodeId: state.plan.scopeNodeId, phasePlanId: state.plan.id,
      issueType: cause === "design" ? "design" : cause === "materialQuality" ? "quality" : "defect", severity,
      priority: severity === "critical" ? "critical" : "normal", status: "open",
      ...
      description: `${cause} requires corrective work in the affected scope.`, reportedAt: at(this.date, purchaseOrderLineId ? 9 : 17),
      reportedById: state.project.siteManagerId, assignedToId: state.assignments[0]!.personId, dueAt: at(addDays(this.date, severity === "critical" ? 3 : 10)),
      estimatedCostImpact: round(hours * state.labour[0]!.budgetRegularHourlyCost + material * budget.budgetUnitCost, 2), estimatedDelayHours: hours };
    this.data.issues.push(issue);
    this.data.issueCauses.push({ id: this.ctx.ids.next("issueCause"), issueId: issue.id, category: cause, status: "confirmed", primary: true,
    ...
    if (severity === "critical") this.data.otherCostEntries.push({ id: this.ctx.ids.next("otherCost"), projectId: state.project.id,
      phasePlanId: state.plan.id, date: this.date, category: "inspection", description: "Specialist inspection after critical defect", amount: 2500, reason: "defectResolution", issueId: issue.id });
  }
  ...
  private delay(state: PhaseState, cause: DelayCauseType, hours: number, links: Partial<DelayEvent> = {}): DelayEvent {
    ...
    if (!existing) {
      const event: DelayEvent = { id: this.ctx.ids.next("delay"), projectId: state.project.id, scopeNodeId: state.plan.scopeNodeId,
        affectedPhasePlanIds: [state.plan.id], reportedAt: at(this.date, 17), startedAt: at(this.date, 9), status: "active",
        impactType: cause === "prerequisite" || cause === "weather" ? "fullStop" : cause === "issue" ? "rework" : "reducedCapacity",
        causeType: cause, estimatedLostHours: 0, estimatedScheduleImpactDays: 0,
        description: `${state.plan.name}: ${cause === "other" ? "shared crew capacity unavailable" : cause}`, ...links };
      existing = { event, lastDate: this.date }; state.openDelays.set(key, existing); this.data.delayEvents.push(event);
    }
    if (!this.touchedDelays.has(touchedKey)) {
      existing.lastDate = this.date; existing.event.estimatedLostHours = round(existing.event.estimatedLostHours + hours);
      existing.event.estimatedScheduleImpactDays++;
      this.data.delayImpactReports.push({ id: this.ctx.ids.next("delayImpact"), delayEventId: existing.event.id, date: this.date,
  ...
  private materialDelay(state: PhaseState, budget: MaterialBudgetLine, hours: number): DelayEvent {
    ...
    return this.delay(state, pending && pending.line.expectedDeliveryDate < this.date ? "lateDelivery" : "materialShortage", hours,
      { materialTypeId: budget.materialTypeId, purchaseOrderLineId: pending?.line.id, supplierId: pending?.order.supplierId });
  }
  private closeDelays(state: PhaseState): void {
    for (const [key, open] of state.openDelays) if (!this.touchedDelays.has(`${state.plan.id}/${key}`) || state.finished) {
      open.event.endedAt = at(this.date, state.finished ? 17 : 9); open.event.status = "ended"; state.openDelays.delete(key);
    }
  }
  ...
  private forecastAndReport(): void {
    for (const state of this.order) {
      ...
      if (!state.lastReportDate || status !== state.lastReportedStatus || progress === 100 ||
        ...
        this.data.progressReports.push({ id: this.ctx.ids.next("progressReport"), siteReportId: this.siteReport(state), projectId: state.project.id,
          phasePlanId: state.plan.id, reportedAt: at(this.date, 18), percentComplete: progress, executionStatus: status, forecastFinishDate: state.forecastFinish });
        state.lastReportDate = this.date; state.lastReportedProgress = progress; state.lastReportedStatus = status;
      }
    }
  }
    ...
    for (const [projectId, milestones] of this.milestoneGroups) {
      ...
      for (const milestone of milestones) {
        ...
        if (!previous || previous.status !== status || Math.abs(daysBetween(previous.forecastDate!, forecast)) >= 3) {
          const report = { id: this.ctx.ids.next("milestoneReport"), milestonePlanId: milestone.id,
            reportedAt: at(this.date, 19), status: status as "complete" | "atRisk" | "onTrack", forecastDate: forecast,
            completedAt: finished ? at(maxDate(...states.map(s => s.finished!)), 18) : undefined };
          this.data.milestoneReports.push(report); this.latestMilestones.set(milestone.id, report);
        }
      }
    }
  }
