import type { CivilDate, Id, ScenarioData } from "../domain/model.ts";
import { addWorkingDays, civilDate, dateOf, isWorkingDay, isoWeek } from "../domain/calendar.ts";
import { byId, compare, groupBy, sum } from "../domain/collections.ts";
import { topologicalPhases } from "../domain/dependencies.ts";
import { costLabour } from "../domain/projections.ts";

export class ScenarioValidationError extends Error {
  constructor(readonly violations: string[]) { super(`Scenario validation failed (${violations.length}):\n${violations.slice(0, 30).join("\n")}`); }
}
...
export function validateScenario(data: ScenarioData): void {
  const errors: string[] = [];
  const check = (valid: unknown, message: string): void => { if (!valid) errors.push(message); };
  ...
  for (const [name, rows] of tables) for (const row of rows) {
    check(!registry.has(row.id), `Duplicate ID ${row.id}`); registry.set(row.id, name);
    for (const [key, value] of Object.entries(row)) {
      ...
      if (typeof value === "string" && /^\d{4}-\d{2}-\d{2}/.test(value)) {
        try { civilDate(value.slice(0, 10)); } catch { errors.push(`${row.id}.${key} has invalid date`); }
  ...
  const foreignKeys: Record<string, string> = {
    projectId: "projects", personId: "people", supervisorId: "people", personnelTypeId: "personnelTypes", materialTypeId: "materialTypes",
    supplierId: "suppliers", customerId: "customers", phaseTypeId: "phaseTypes", projectManagerId: "people", siteManagerId: "people",
    workingCalendarId: "workingCalendars", projectPlanId: "projectPlans", scopeNodeId: "scopeNodes", parentId: "scopeNodes",
    phasePlanId: "phasePlans", predecessorPhaseId: "phasePlans", successorPhaseId: "phasePlans", blockingPhasePlanId: "phasePlans",
    reporterId: "people", reportedById: "people", assignedToId: "people", createdById: "people", siteReportId: "siteReports",
    issueId: "issues", delayEventId: "delayEvents", purchaseOrderId: "purchaseOrders", purchaseOrderLineId: "purchaseOrderLines",
    deliveryId: "deliveries", absenceId: "personnelAbsences", milestonePlanId: "milestonePlans",
  };
  ...
  const projects = byId(data.projects), scopes = byId(data.scopeNodes), phases = byId(data.phasePlans), plans = byId(data.projectPlans);
  const calendars = byId(data.workingCalendars), issues = byId(data.issues), delays = byId(data.delayEvents);
  const deliveries = byId(data.deliveries), orderLines = byId(data.purchaseOrderLines), orders = byId(data.purchaseOrders);
  ...
  try { topologicalPhases(data.phasePlans, data.phaseDependencies); } catch (e) { errors.push(String(e)); }
  for (const scope of data.scopeNodes) {
    const seen = new Set([scope.id]);
    let parent = scope.parentId;
    while (parent) {
      if (seen.has(parent)) { errors.push(`Scope cycle at ${scope.id}`); break; }
      seen.add(parent); const node = scopes.get(parent)!;
      check(node.projectId === scope.projectId, `${scope.id}: parent belongs to another project`); parent = node.parentId;
    }
  }
  ...
  const checkAttribution = (row: { id: string; projectId: Id<"project">; phasePlanId?: Id<"phase">; issueId?: Id<"issue">; delayEventId?: Id<"delay"> }, date: CivilDate): void => {
  ...
  for (const row of data.labourEntries) {
    ...
    daily.set(dayKey, (daily.get(dayKey) ?? 0) + row.hours); weekly.set(weekKey, (weekly.get(weekKey) ?? 0) + row.hours);
    if (row.activityReason === "plannedWork") {
      const old = firstWork.get(row.phasePlanId); if (!old || old > row.workDate) firstWork.set(row.phasePlanId, row.workDate);
    }
  }
  ...
  try { costLabour(data); } catch (e) { errors.push(String(e)); }
  ...
  for (const line of data.deliveryLines) {
    ...
    transactions.push({ key: `${order.projectId}/${orderLine.materialTypeId}`, time: delivery.deliveredAt, id: line.id, quantity: line.quantityAccepted, unitCost: orderLine.unitCost });
  }
  for (const row of data.materialUsageEntries) {
    ...
    transactions.push({ key: `${row.projectId}/${row.materialTypeId}`, time: row.usedAt, id: row.id, quantity: -row.quantity, unitCost: row.unitCostApplied });
  ...
  for (const tx of transactions.sort((a, b) => compare(a.time, b.time) || compare(a.id, b.id))) {
    ...
    check(quantity >= -0.001, `${tx.id}: negative material inventory (${quantity})`); inventory.set(tx.key, quantity);
  }
