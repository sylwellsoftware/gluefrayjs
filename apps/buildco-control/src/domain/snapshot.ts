...
export function scenarioAtDate(data: ScenarioData, date: CivilDate | string): ScenarioData {
  ...
  const projects = data.projects.map((p): Project => ({ ...p, status: (phases.get(p.id) ?? []).every(phase => completed.has(phase.id)) ? "completed"
    : startedProjects.has(p.id) || p.plannedStart <= cutoff ? "active" : "planned" }));
  const issues = data.issues.filter(i => through(i.reportedAt)).map(i => ({ ...i,
  ...
  const delayEvents = data.delayEvents.filter(d => through(d.reportedAt)).map(d => {
    ...
    return { ...d, endedAt: ended ? d.endedAt : undefined, status: ended ? "ended" as const : "active" as const,
  ...
  return { ...data, metadata: { ...data.metadata, asOfDate: cutoff }, projects, progressReports, labourEntries,
    personnelAssignments: data.personnelAssignments.filter(a => !a.createdAt || through(a.createdAt)).map(a => ({ ...a,
    ...
    issues, issueCauses: data.issueCauses.filter(c => issueIds.has(c.issueId)), deliveries, deliveryLines, purchaseOrderLines,
    purchaseOrders: orders.map(p => {
      ...
      return { ...p, status: p.status === "cancelled" ? "cancelled" : received >= ordered - 0.0001 ? "delivered" : received > 0 ? "partDelivered" : "ordered" };
    }),
    otherCostEntries: data.otherCostEntries.filter(e => e.date <= cutoff), delayEvents, delayImpactReports,
