...
function serial(date: CivilDate): number {
    ...
    if (result === undefined) {
        ...
        if (serials.size > 30000) {
            serials.clear();
            dates.clear();
        }
        serials.set(date, result);
...
export function dateOf(instant: Instant): CivilDate {
    return instant.slice(0, 10) as CivilDate;
}
...
export function addDays(date: CivilDate, days: number): CivilDate {
    ...
    if (result === undefined) {
        ...
        dates.set(day, result);
        serials.set(result, day);
    }
    return result;
}
...
export function isoWeek(date: CivilDate): CivilDate {
    return addDays(date, 1 - weekday(date));
}

export function maxDate(...dates: CivilDate[]): CivilDate {
    return dates.reduce((a, b) => a > b ? a : b);
}

export function minDate(...dates: CivilDate[]): CivilDate {
    return dates.reduce((a, b) => a < b ? a : b);
}
