...
export function analyzeScenario(corpus: ScenarioData, date: CivilDate = corpus.metadata.anchorDate): ScenarioAnalysis {
    ...
    const addIssueCost = (id: Id<"issue"> | undefined, cost: number): void => {
        if (id) issueCosts.set(id, (issueCosts.get(id) ?? 0) + cost);
    };
    ...
    const delayByPhase = groupBy(data.delayEvents.flatMap(d => d.affectedPhasePlanIds.map(id => ({
        id,
        event: d
    }))), d => d.id);
    ...
    for (const phase of topologicalPhases(data.phasePlans, data.phaseDependencies)) {
        ...
        const health: PhaseHealth = {
            ...
            plannedProgressAtDate: plannedProgress(phase, date, data),
            progressVariance: round(progress - plannedProgress(phase, date, data)),
            plannedStart: phase.plannedStart,
            plannedFinish: phase.plannedFinish,
            forecastStart,
            forecastFinish,
            scheduleVarianceDays: workingDaysBetween(phase.plannedFinish, forecastFinish, calendar),
            prerequisiteBlocked: blockingPhaseIds.length > 0,
            blockingPhaseIds,
            budgetLabourHours: totalHours,
            actualLabourHours: sum(actualLabour, l => l.entry.hours),
            overtimeHours: sum(actualLabour, l => l.overtimeHours),
            budgetLabourCost,
            actualLabourCost,
            budgetMaterialCost,
            actualMaterialCost,
            ...
            costVariance: actualDirectCost - expectedCostAtProgress,
            plannedHeadcount: headcount,
            assignedHeadcount: progress === 100 ? 0 : assigned,
            ...
            materialShortageCount: shortage,
            lateMaterialCount: late,
            openIssueCount: open.length,
            overdueIssueCount: open.filter(i => i.dueAt && dateOf(i.dueAt) < date).length,
            criticalIssueCount: open.filter(i => i.severity === "critical").length,
    ...
    return {
        date, phases: [...results.values()], labour, issueCosts, projects: data.projects.map(project => {
            ...
            return {
                ...
                budgetCost: sum(phases, p => p.totalBudgetCost) + extraBudget,
                actualCost: sum(phases, p => p.actualDirectCost) + extraActual,
...
export const HEALTH_THRESHOLDS = {
    overtimeRatio: 0.15,
    staleDays: 7,
    costOverrunRatio: 0.05,
    highRiskDelayDays: 10
} as const;
...
export function classifyHealth(health: PhaseHealth): Record<"behindSchedule" | "overBudget" | "highOvertime" | "understaffed" | "materialShortage" | "lateMaterial" | "openDefects" | "blocked" | "staleReporting" | "highRisk", boolean> {
    ...
    return {
        behindSchedule: health.scheduleVarianceDays > 0,
