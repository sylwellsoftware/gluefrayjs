...
import { analyzeScenario, classifyHealth } from "./domain/projections.ts";
import { scenarioAtDate } from "./domain/snapshot.ts";
import { civilDate } from "./domain/calendar.ts";
import { round, sum } from "./domain/collections.ts";
import type { ScenarioGenerationOptions } from "./domain/model.ts";
import { assessScenarioCoverage } from "./generator/coverage.ts";
...
if (values.help) {
  ...
  try {
    ...
    const options: ScenarioGenerationOptions = { ...DEFAULT_SCENARIO, profile: values.profile as ScenarioGenerationOptions["profile"],
      seed: values.seed === undefined ? DEFAULT_SCENARIO.seed : Number(values.seed), anchorDate: values.anchor ?? DEFAULT_SCENARIO.anchorDate,
    ...
    if (values.json) console.log(JSON.stringify(snapshot));
      ...
      console.log(JSON.stringify({ scenario: snapshot.metadata, generationAndValidationMs: generatedMs,
        ...
        projects: analysis.projects.map(p => ({ name: snapshot.projects.find(project => project.id === p.projectId)!.name, ...p,
          budgetCost: round(p.budgetCost, 2), actualCost: round(p.actualCost, 2), costVariance: round(p.costVariance, 2) })),
        phaseConditions: Object.fromEntries(Object.keys(traits[0] ?? {}).map(key => [key, traits.filter(t => t[key as keyof typeof t]).length])),
        totalHours: round(sum(analysis.labour, l => l.entry.hours), 2), overtimeHours: round(sum(analysis.labour, l => l.overtimeHours), 2),
      }, null, 2));
    }
