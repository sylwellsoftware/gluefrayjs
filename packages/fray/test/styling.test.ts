...
describe('style registry', () => {
    test('injects and updates one structural stylesheet per document', () => {
        ...
        assert.match(second.textContent, /\.fray-layout-horizontal,\s*\.fray-layout-vertical\s*\{[^}]*display:\s*flex[^}]*align-items:\s*stretch[^}]*min-inline-size:\s*0[^}]*min-block-size:\s*0/)
        assert.match(second.textContent, /\.fray-layout-horizontal\s*\{[^}]*flex-direction:\s*row/)
        assert.match(second.textContent, /\.fray-layout-vertical\s*\{[^}]*flex-direction:\s*column/)
        assert.match(second.textContent, /\.fray-size-natural\s*\{[^}]*flex:\s*0 0 auto/)
        assert.match(second.textContent, /\.fray-size-flexible\s*\{[^}]*flex:\s*1 1 0[^}]*min-inline-size:\s*0[^}]*min-block-size:\s*0/)
        assert.match(second.textContent, /\.fray-scroll\s*\{[^}]*overflow:\s*auto[^}]*min-inline-size:\s*0[^}]*min-block-size:\s*0/)
        assert.doesNotMatch(second.textContent, /\.fray-(?:layout|size|scroll)[^{]*\{[^}]*!important/)
    ...
    test('collects the complete Panel treatment without unrelated component CSS', () => {
        ...
        assert.match(stylesheet, /fray-panel > fray-layout\.panel-content/)
    ...
    test('collects the complete SplitView treatment without unrelated component CSS', () => {
        ...
        assert.match(stylesheet, /fray-splitview > fray-primary\s*\{[^}]*flex:\s*0 0 var\(--split-primary-size, 40%\)/)
        assert.match(stylesheet, /fray-splitview > fray-secondary\s*\{[^}]*flex:\s*1 1 0/)
        assert.match(stylesheet, /fray-splitview > fray-separator\s*\{[^}]*touch-action:\s*none/)
        assert.match(stylesheet, /fray-splitview\.horizontal > fray-separator\s*\{[^}]*cursor:\s*col-resize/)
        assert.match(stylesheet, /fray-splitview\.vertical > fray-separator\s*\{[^}]*cursor:\s*row-resize/)
