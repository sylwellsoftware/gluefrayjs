...
test('History routing restores an outlet, preserves native links, and cleans up', async ({page}) => {
    await page.goto('/browser-second?routing=true')
    await page.waitForFunction(() => globalThis.frayTestReady === true)
    const navigation = page.getByRole('navigation', {name: 'Browser routes'})
    const first = navigation.getByRole('link', {name: 'First route'})
    const second = navigation.getByRole('link', {name: 'Second route'})
    await expect(second).toHaveAttribute('aria-current', 'page')
    await expect(page.getByText('Second page')).toBeVisible()
    await expect(page.getByText('First page')).toHaveCount(0)
    ...
    await expect(page.getByText('Second page')).toHaveCount(0)
    await expect(page).toHaveURL(/\/browser-first$/)
