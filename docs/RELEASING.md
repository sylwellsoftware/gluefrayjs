...
2. Run `pnpm verify:release` and review
   `.artifacts/release/package-artifacts.json` plus both tarball inventories.
...
5. Approve the protected `npm-release` GitHub environment. The workflow reruns
   release verification and uses npm OIDC to stage the exact tarballs. Glue is
   staged before Fray when both are selected. No npm token is stored in GitHub.
...
approve and verify Glue before approving Fray. Verify the public version,
distribution tag, clean exact-version installation, and npm provenance linked
to this repository.

## Failure and recovery

If Glue stages but Fray fails, do not rerun blindly. Inspect the pending Glue
stage, correct the cause, and either stage only Fray or reject Glue with `npm
...
the trusted-publisher entries for both packages. The intended trust identity is
